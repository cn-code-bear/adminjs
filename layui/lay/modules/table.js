...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util','treetable'], function(exports){
  "use strict";
  var statusTPL=function(id,data){
	  let TPL=``;
	  data.forEach(function(item,index){
		TPL=TPL+`<li class="${index==0?'layui-this':''}">${item.title}</li>`;    
	  })
	  return `<div class="page-menu layui-tab layui-tab-brief" lay-filter="${id}" style="background: #fff;margin: 0;">
	    <ul class="layui-tab-title">
		  ${TPL}
	    </ul>
	  </div>`;
  }
  var userTPL=function(d){
  	  return `<div class="table-user-info">
			<div class="header">
			 <img src="${d.user_header||'/style/img/dimg.png'}" />
			</div>
			<div class="info">
			<p>${d.user_name||'--'}</p>
			<p>用户ID:${d.id||'--'}</p>
  ...
  var searchTpl=function(id,data=[]){
	  let fieldTpl='',toolTpl='';
	  for (var item of data.cols) {    
	  fieldTpl=fieldTpl+`<div class="page-search-field layui-col-md3">
	       <label class="page-search-field-label">${item.title}</label>
	       <div class="page-search-field-input">
	         <input type="${item.type||'text'}" url="${item.url||''}" data="${item.data||''}" name="${item.field}" placeholder="${item.msg||'请输入'}" autocomplete="off" class="layui-input">
	       </div>
	     </div>`;  
	  }
	  if(data.cols.length>3){
		toolTpl=`<a class="page-search-tool-all">展开<i class="layui-icon layui-icon-down"></i></a>`;    
	  }
	  return `
  <form class="layui-form page-search">
  <div class="layui-row layui-col-space12">
  ${fieldTpl}
  	      <div class="page-search-tool layui-col-md3">
  			<button  class="layui-btn layui-btn-primary page-search-tool-reset" type="reset">重置</button>
  	        <button class="layui-btn" lay-submit lay-filter="${id}">
  	          查询
  	        </button>
  			${toolTpl}
  	      </div>
  ...
  var infoTpl=function(data=[]){
  	  let fieldTpl='',toolTpl='';
  	  for (var item of data.cols) {    
  	  fieldTpl=fieldTpl+`<div class="item">
  	       <span>文章总数</span>
  	  		   <p>100</p>
  	     </div>`;  
  	  }
  ...
  var formTpl=function(id,data=[],edit={}){
  	  let fieldTpl='';
  		  if(edit.id){
  			fieldTpl='<input type="hidden" name="id" value='+edit.id+'>';  
  		  }
  	  for (var item of data) {
		  ...
		  if(item.templet){  
		   laytpl($(item.templet).html()).render(edit,function(html){
			input=html;   
		   });		 
		  }
  		  else if(item.type=='edit'){
			  
  			 input=`<textarea  ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} type='edit' name="${item.field}" placeholder="${item.msg||'请输入'}">${edit[item.field]||''}</textarea>`;
			...
			for (var radio of item.data) {
			input=input+`<input  lay-filter="${item.field}" ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} type="radio" name="${item.field}" value="${radio[1]}" title="${radio[0]}" ${radio[1]==edit[item.field]?'checked':''}>`;
			} 
		  }
		  else if(item.type=='select'){
			        input=`<select name="${item.field}" ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''}>
                      <option value=""></option>`;
		  			for (var select of item.data) {
		  			input=input+`<option ${select[1]==edit[item.field]?'selected':''} value="${select[1]}">${select[0]}</option>`;
		  			} 
					input=input+`</select>`;
		  }
		  else if(item.type=='text'){
		  			  
		     input=`<textarea ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} class="layui-textarea" name="${item.field}" placeholder="${item.msg||'请输入'}" ${readonly}>${edit[item.field]||''}</textarea>`;
		  }
		  else{
  			  input=`<input type="${item.type||'text'}" name="${item.field}" value="${edit[item.field]||''}" placeholder="${item.msg||'请输入'}" url="${item.url||''}" data="${item.data||''}" ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} autocomplete="off" class="layui-input" ${readonly}>`; 
  		  }	
  	  fieldTpl=fieldTpl+`<div class="layui-form-item">
  		    <label class="layui-form-label">${item.title}</label>
  			<div class="layui-input-${item.end?'inline':'block'}" ${item.end?'style="min-width:320px"':''}>
  			${input}
  			 </div>
			 ${item.end?'<div class="layui-form-mid layui-word-aux">'+item.end+'</div>':''}
  	     </div>`;  
  	  }
  	  return `
  <div class="layui-form" wid100 style="padding:32px">
  ${fieldTpl}
              <div class="layui-form-item">
              <div class="layui-input-block">
                <button class="layui-btn" lay-submit  lay-filter="${id}">提交</button>
              </div>
            </div>
          </div>`;
  };
  ...
  ,TPL_HEADER = function(options){
    ...
    return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
      ,'lay-skin="line" {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
      ,'<thead>'
      ,'{{# layui.each(d.data.cols, function(i1, item1){ }}'
        ...
        ,'{{# layui.each(item1, function(i2, item2){ }}'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" data-key="{{d.index}}-{{i1}}-{{i2}}" {{# if( item2.parentKey){ }}data-parentkey="{{ item2.parentKey }}"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize || item2.colGroup){ }}data-unresize="true"{{# } }} class="{{# if(item2.hide){ }}layui-hide{{# } }}{{# if(isSort){ }} layui-unselect{{# } }}{{# if(!item2.field){ }} layui-table-col-special{{# } }}">'
            ,'<div class="layui-table-cell a1 laytable-cell-'
              ,'{{# if(item2.colGroup){ }}'
                ,'group'
              ,'{{# } else { }}'
                ,'{{d.index}}-{{i1}}-{{i2}}'
  ...
  ,TPL_MAIN = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    ...
    ,'<div class="layui-table-tool">'
      ,'<div class="layui-table-tool-temp">{{d.data.title||"数据列表"}}</div>'
  ...
  Class.prototype.render = function(){
    var that = this
    ,options = that.config;
	let id=location.hash.replace(/\//g,"-").substr(2);
	if(!options.elem){
	$("table").attr('id',id);
	...
	if(options.search && !$(".page-search").text()){
	table.search(id,options.search);	
	}
	if(options.menu && !$(".page-menu").text()){
	//table.search(id,options.search);	
	$(".layui-fluid").before(statusTPL(id+'-tabs',options.menu));
	layui.element.on('tab('+id+'-tabs)', function(e){
	   table.reload(id,{
	      url: options.url
	     ,where: options.menu[e.index].data
	     ,page: {
	     curr: 1 //重新从第 1 页开始
	    }
	   });
	...
	if(options.add || options.toolEvent){
		table.on('toolbar('+id+')', function(obj){
		  for (let key in options.toolEvent) {
		  	if(obj.event==key){
			options.toolEvent[key](obj)		
			}
		  }	
		  if(obj.event=='add'){
		layer.open({
		  ...
		  //isOutAnim:false,
		  area: ['75vw', '75vh'],	  
		  content: formTpl(id+'-add',options.add.cols),
							  success: function(layero, index){
							   form.render()
							   form.on('submit('+id+'-add'+')', function(data){
							   	  layui.admin.req({
							   	       type: "POST",  
							   	    url: options.add.url
							   	    ,data: data.field
							   	    ,success: function(res){
							   	    	if(res.code==0){
										 if(options.add.success){
										 layer.close(index);	 
										 options.add.success(res.data); 	 
										 }else{
	...
	if(options.edit || options.events){
		...
		table.on('tool('+id+')', function(obj){
		  ...
		  var tr = obj.tr;
		  if(options.events){
			  for(var index in options.events){
				if(layEvent == index){
					if(typeof options.events[index] == 'object'){
					let events=options.events[index]; 	
					layer.open({
					  type: 1, 
					  title:events.title,
					  shadeClose:true,
					  ...
					  //isOutAnim:false,
					  area: ['75vw', '75vh'],	  
					  content: formTpl(id+layEvent,events.cols,data),
					  success: function(layero, index){
								form.render();
								form.on('submit('+id+layEvent+')', function(data){
									  layui.admin.req({
									       type: "POST",  
									    url: events.url
									    ,data: data.field
					...
					options.events[index](data,tr,obj)	
					}
				}
			  }
		  }
		  ...
		  if(layEvent === 'edit'){ //查看
		    layer.open({
		      type: 1, 
		      title:options.edit.title,
		      shadeClose:true,
		      anim:-1,
		      //isOutAnim:false,
		      area: ['75vw', '75vh'],	  
		      content: formTpl(id+'-edit',options.edit.cols,data),
		      success: function(layero, index){
						form.render();
						form.on('submit('+id+'-edit'+')', function(data){
							  layui.admin.req({
							       type: "POST",  
							    url: options.edit.url
							    ,data: data.field
	...
	if(options.del){
	table.bottomBar(id,options.del);	
	}
    ...
    if(typeof options.defaultToolbar === 'object'){
      layui.each(options.defaultToolbar, function(i, item){
        ...
        if(thisItem){
			let btn='';
		  if(item!='add'){
			btn=' layui-btn-primary';  
		  }	
          iconElem.push('<div class="layui-btn'+btn+'" title="'+ thisItem.title +'" lay-event="'+ thisItem.layEvent +'">'
            +'<i class="layui-icon '+ thisItem.icon +'"></i>'+thisItem.title
          +'</div>');
        }
      });
    }
  ...
  Class.prototype.setColsWidth = function(){
    ...
    //减去边框差和滚动条宽
    //cntrWidth = cntrWidth - that.getScrollWidth(that.layMain[0]);
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
	  ...
	  if(options.cellHeight){
	   cellHeight=' style="height:'+ options.cellHeight +'px;line-height:'+options.cellHeight+'px"';
	  }		
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            ...
            ,'<div class="layui-table-cell  laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              switch(item3.type){
                ...
                break;
				case 'status':
				  return `<font lay-event="status">
				     <input type="checkbox" ${tplData['status']==1?'checked':''} lay-skin="switch" lay-filter="status" title="状态" lay-text="是|否" value="${tplData['id']}">
					 </font>`;
				case 'user':
				  return userTPL(tplData['user']);	 
				break;
              };
              
              //解析工具列模板
              if(item3.toolbar){
				if(item3.toolbar instanceof Array){
				let TPL=``;	
				item3.toolbar.forEach(function(item,index){
				TPL=TPL+`<a lay-event="${item[1]}">${item[0]}</a>`;	
				if(index<item3.toolbar.length-1){
				TPL=TPL+`<em></em>`;	
				}
				})	
				return TPL;	
    ...
    if(options.page){
      options.page = $.extend({
        ...
        ,count: count
        ,limit: options.limit
        ,limits: options.limits || [15,30,50,100,200]
        ,groups: 3
  ...
  //表格导出
  table.exportFile = function(id, data,title='数据'){
    var that = this;
    ...
    alink.download = title +'_'+data.length+'.' + type; 
  ...
  //自动完成渲染
  table.init();
  table.bottomBar=function(id,url){
	 table.on('checkbox('+id+')', function(obj){
	 	...
	 	layer.open({
	 	  ...
	 	  anim:-1,
	 	  isOutAnim:false,
	 	  area: ['calc(100% - 180px)', '50px'],	  
	 	  content: `<div class="table-toolbar-bottom">
		  <div style="width:30%;float:left;text-align:left">
		  <a onclick="layui.table.nocheck('${url}')" style="padding-left:24px" >取消选择</a>
		  </div>
		  <div style="width:70%;float:left">
		  <span style="padding-right:24px">已选择<font style="padding:0 4px;color:#FF5722">${checkData.length}</font>项</span>
		  <button class="layui-btn layui-btn-danger layui-btn-danger layui-btn-sm" onclick="layui.table.del('${url}')">批量删除</button>
		  </div>
		  </div>`
	 	});	
	 	}else{
  ...
  table.post=function(url,data={},callback){
	  var that=this;
	  view.reg({
	    type: 'post'
  ...
  table.nocheck= function(api){
	  $("input[type='checkbox'][name='layTableCheckbox']").prop('checked', false);
	  form.render("checkbox");
	  layer.closeAll();
	  let id=$("table").attr('id');
	  let data=table.cache[id];
	  layui.each(data, function(i, item){
		table.cache[id][i][table.config.checkName]=false; 
	  });
  }
  table.del= function(api){
	  let id=$("table").attr('id');
	  console.log(id);
	   ...
	   layer.confirm('确定删除吗？', function(index) {
		table.post(api,{id:delList},function(res){
			if(res.code==0){
			 layui.admin.events.refresh();
			 layer.closeAll();
			 layer.msg(res.msg); 
  ...
  table.search=function(id,data){
	  $(".page-table").before(searchTpl(id+'-search',data));
	  // $(".page-search-tool-reset").on('click',function(){
	  //  $(".layui-form")[0].reset();	
	  // })
	  $(".page-search-tool-all").on('click',function(){
	  	var num=$(".page-search-field").length;
	  	num=9-num%4*3;
	  	if($(".page-search-tool-all i").hasClass('layui-icon-down')){
	  	$(".page-search-field").css({'display':'flex'});
	  	$(".page-search-tool").addClass('layui-col-md-offset'+num);
	  	$(".page-search-tool-all").html('收起<i class="layui-icon layui-icon-up"></i>');	
	  	}else{
	  	$(".page-search-field:gt(2)").css({'display':'none'});
	  	$(".page-search-tool").removeClass('layui-col-md-offset'+num);
	  	$(".page-search-tool-all").html('展开<i class="layui-icon layui-icon-down"></i>');		
	  	}
	  });
	  form.render();
	  form.on('submit('+id+'-search)', function(data){
		table.reload(id,{where:data.field});
	    return false;
	  });
  }
