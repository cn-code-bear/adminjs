layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" class="layim-'+ (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') +'"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ,'<div class="layui-layim-user"><img height="20" style="padding-right:5px" src="{{ d.mine.avatar }}">{{ d.mine.username }}</div>'
    ,'</div>'
    ...
    ,'{{# } }}">'
      ...
      ,' layim-this'
      ,'{{# } }}'
      ,'" title="部门" layim-event="tab" lay-type="group">&#xe613;</li>'
    ...
    ,'{{# layui.each(d.friend, function(index, item){ var spread = d.local["spread"+index]; }}'
      ,'<li>'
        ,'<h5 layim-event="spread" lay-type="{{ spread }}"><i class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {  }}&#xe602;{{# } }}</i><span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite class="layim-count"> {{ item.list.length||0 }}</cite>)</em></h5>'
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="../admin/style/img/index/im.png"><span>'+ (content||cache.base.title||'IM工作台') +'</span>'
  ...
  var anim = 'layui-anim-upbit', events = {
      ...
      if(othis.data('down')){
        ...
        events.groupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', othis, {
          ...
          ,success: function(layero){
            ...
            var friend=Object.values(cache.friend);
            for(var i=0;i<friend.length;i++){
            if(friend[i].id==thatChat.data.id){
            var list=friend[i].list;	
            }	
            }
           ...
           // post(members, function(res){
              ...
              //获取群员
              othis.find('.layim-chat-members').html((list||[]).length);
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        ,success: function(layero, index){
          layui.view(this.id).render(cache.base.chatLog,thatChat.data).done(function(){
