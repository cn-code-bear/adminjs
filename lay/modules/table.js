...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util','treetable'], function(exports){
  ...
  var formTpl=function(id,data=[],edit={}){
  	  ...
  	  for (var item of data) {
		  ...
		  if(item.readonly){
			...
			input=input+`<input ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} type="radio" name="${item.field}" value="${radio[1]}" title="${radio[0]}" ${radio[1]==edit[item.field]?'checked':''}>`;
			} 
		  }
		     ...
		     input=`<textarea ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} class="layui-textarea" name="${item.field}" placeholder="${item.msg||'请输入'}" ${readonly}>${edit[item.field]||''}</textarea>`;
  			  ...
  			  input=`<input type="${item.type||'text'}" name="${item.field}" value="${edit[item.field]||''}" placeholder="${item.msg||'请输入'}" url="${item.url||''}" ${item.verify?'lay-verify="required" lay-reqText="'+item.title+'不能为空"':''} autocomplete="off" class="layui-input" ${readonly}>`; 
  		  }	
		...
		table.on('tool('+id+')', function(obj){
		  ...
		  if(options.events){
			  for(var index in options.events){
				if(layEvent == index){
					console.log(options.events[index]);
  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ //Ajax请求
      ...
      data.token=layui.data(layui.setter.tableName)[layui.setter.request.token];
      that.loading();

      $.ajax({
        type: options.method || 'post'
        ,url: layui.setter.apiurl+options.url
